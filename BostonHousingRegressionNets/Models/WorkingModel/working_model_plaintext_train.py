...
def working_model_plaintext_train(x_train, y_train):
    # Parameters:
    # Base Params:
    ...
    with tf.Session(config=EncryptionParameters.config) as sess:
        ...
        for epoch_iteration in range(TrainingParameters.num_epochs):
            ...
            if epoch_iteration % TrainingParameters.checkpoint_frequency == 0:
                np.save(training_results_numpy_save_dir + f"{TrainingParameters.training_output_numpy_file_path}{epoch_iteration}", training_output)
