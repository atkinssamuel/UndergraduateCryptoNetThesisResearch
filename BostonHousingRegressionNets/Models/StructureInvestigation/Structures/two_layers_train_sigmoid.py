...
def two_layers_train_sigmoid(x_train, y_train):
    print("two_layers_train_sigmoid")
    # Parameters:
    # Base Params:
    ...
    with tf.Session(config=EncryptionParameters.config) as sess:
        ...
        sess.close()
    with open(results_dir + "TwoLayers/Sigmoid/" + f'training_output_mean.csv', 'a') \
            as fd:
        fd.write(f"{np.abs(np.mean(training_output))}, ")
    with open(results_dir + "TwoLayers/Sigmoid/" + f'initial_training_loss.csv', 'a') \
            as fd:
