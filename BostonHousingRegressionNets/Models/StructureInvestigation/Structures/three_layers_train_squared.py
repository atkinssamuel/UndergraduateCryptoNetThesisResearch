...
def three_layers_train_squared(x_train, y_train):
    print("three_layers_train_squared")
    # Parameters:
    # Base Params:
    ...
    y3 = tf.math.square(tf.matmul(y2, W3) + b3)
    ...
    with tf.Session(config=EncryptionParameters.config) as sess:
        ...
        sess.close()
    with open(results_dir + "ThreeLayers/Squared/" + f'training_output_mean.csv', 'a') \
            as fd:
        fd.write(f"{np.abs(np.mean(training_output))}, ")
    with open(results_dir + "ThreeLayers/Squared/" + f'initial_training_loss.csv', 'a') \
            as fd:
