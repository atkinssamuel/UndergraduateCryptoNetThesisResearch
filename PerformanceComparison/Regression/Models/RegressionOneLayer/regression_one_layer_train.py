...
def regression_one_layer_train(x_train, y_train, x_valid, y_valid):
    print("regression_one_layer_train")
    # Parameters:
    # Base Params:
        ...
        for epoch_iteration in range(TrainingParameters.num_epochs):
            for batch in range(int(sample_count / TrainingParameters.batch_size)):
                ...
                if batch == 250 and encrypted_flag == 1:
                    ...
                    print(f"Time Elapsed = {time_elapsed}s, Current Batch = {batch}")
                    return
