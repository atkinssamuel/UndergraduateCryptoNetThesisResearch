...
def regression_two_layers_plaintext_train(x_train, y_train, x_valid, y_valid):
    print("regression_two_layers_plaintext_train")
    # Parameters:
    # Base Params:
    ...
    with tf.Session(config=EncryptionParameters.config) as sess:
        ...
        for epoch_iteration in range(TrainingParameters.num_epochs):
            ...
            if epoch_iteration % TrainingParameters.checkpoint_frequency == 0:
                ...
                print(f"Current Validation Loss = {validation_losses[-1][0]}, Min Validation Loss = {validation_losses[min_index][0]} at Epoch {min_index}")
